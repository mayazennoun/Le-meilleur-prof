...
const MarieMartinScreen = () => {
    
    const navigation = useNavigation();
    const [isFavourite,toggleFavourite] = useState(false);
  return (
    <View className = "bg-white flex-1 ">
    <Image source={require("../assets/MarieMartin.jpg")}
    ...
    <FontAwesome name="heart-o" size={24} color={isFavourite? "red" : "white"} />
     ...
     <View  style={{borderTopLeftRadius:40,borderTopRightRadius: 40}} className="px-5  flex flex-1 justify-between bg-white pt-8 -mt-14 ">
      <ScrollView showsVerticalScrollIndicator={false} className="space-y-5">
        ...
        <Text style={{fontSize:15}} className="font-bold flex-1 "> Marie Martin </Text>
        <Text style={{fontSize:15}} className="text-[#BA68C8] font-bold"> 4000DA/Mois</Text>
        </View>
        <Text style={{fontSize:15}} className="tracking-wide mb-2"> Marie Martin enseigne le français avec passion aux étudiants de tous niveaux. Grâce à son expertise et à son dévouement, elle inspire la confiance et la motivation chez ses élèves, les aidant à maîtriser la langue et à s'épanouir dans leur parcours académique.
</Text>
       ...
       <View className="flex space-y-2"> 
       <Text className="font-bold">Français </Text>
...
export default MarieMartinScreen