...
const VincentLaurentScreen = () => {
    
    const navigation = useNavigation();
    const [isFavourite,toggleFavourite] = useState(false);
  return (
    <View className = "bg-white flex-1 ">
    <Image source={require("../assets/Vincent Laurent.jpg")}
    ...
    <FontAwesome name="heart-o" size={24} color={isFavourite? "red" : "white"} />
     ...
     <View  style={{borderTopLeftRadius:40,borderTopRightRadius: 40}} className="px-5  flex flex-1 justify-between bg-white pt-8 -mt-14 ">
      <ScrollView showsVerticalScrollIndicator={false} className="space-y-5">
        ...
        <Text style={{fontSize:15}} className="font-bold flex-1 "> Vincent Laurent  </Text>
        <Text style={{fontSize:15}} className="text-[#BA68C8] font-bold"> 2000DA/Mois</Text>
        </View>
        <Text style={{fontSize:15}} className="tracking-wide mb-2">Vincent Laurent, spécialiste en éducation physique, offre des cours dynamiques et motivants. Sa passion pour le mouvement et son approche pédagogique axée sur le plaisir et la santé inspirent les élèves à explorer leur potentiel athlétique et à adopter un mode de vie actif et équilibré.
</Text>
       ...
       <View className="flex space-y-2"> 
       <Text className="font-bold">Education Physique </Text>
...
export default VincentLaurentScreen