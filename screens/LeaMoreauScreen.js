...
const LeaMoreauScreen= () => {
    
    const navigation = useNavigation();
    const [isFavourite,toggleFavourite] = useState(false);
  return (
    <View className = "bg-white flex-1 ">
    <Image source={require("../assets/Lea Moureau.jpg")}
    ...
    <FontAwesome name="heart-o" size={24} color={isFavourite? "red" : "white"} />
     ...
     <View  style={{borderTopLeftRadius:40,borderTopRightRadius: 40}} className="px-5  flex flex-1 justify-between bg-white pt-8 -mt-14 ">
      <ScrollView showsVerticalScrollIndicator={false} className="space-y-5">
        ...
        <Text style={{fontSize:15}} className="font-bold flex-1 "> Léa Moreau </Text>
        <Text style={{fontSize:15}} className="text-[#BA68C8] font-bold"> 2000DA/Mois</Text>
        </View>
        <Text style={{fontSize:15}} className="tracking-wide mb-2"> Léa Moreau, passionnée de théâtre, offre des cours empreints de créativité et d'expression. Son engagement envers l'art dramatique et son approche pédagogique vivante incitent les élèves à explorer le monde du théâtre avec audace et imagination, leur permettant ainsi de développer leur confiance et leur capacité d'interprétation.
</Text>
       ...
       <View className="flex space-y-2"> 
       <Text className="font-bold">Théatre </Text>
       ...
       <View className="flex space-y-2"> 
       <Text className="font-bold">2000DA/Mois </Text>
...
export default LeaMoreauScreen