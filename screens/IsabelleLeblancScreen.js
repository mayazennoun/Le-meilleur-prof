...
const IsabelleLeblancScreen = () => {
    
    const navigation = useNavigation();
    const [isFavourite,toggleFavourite] = useState(false);
  return (
    <View className = "bg-white flex-1 ">
    <Image source={require("../assets/Isabelle Leblanc.jpg")}
    ...
    <FontAwesome name="heart-o" size={24} color={isFavourite? "red" : "black"} />
     ...
     <View  style={{borderTopLeftRadius:40,borderTopRightRadius: 40}} className="px-5  flex flex-1 justify-between bg-white pt-8 -mt-14 ">
      <ScrollView showsVerticalScrollIndicator={false} className="space-y-5">
        ...
        <Text style={{fontSize:15}} className="font-bold flex-1 "> Isabelle Leblanc </Text>
        <Text style={{fontSize:15}} className="text-[#BA68C8] font-bold"> 2000DA/Mois</Text>
        </View>
        <Text style={{fontSize:15}} className="tracking-wide mb-2">  Isabelle Leblanc, enseignante en biologie, propose des cours passionnants et instructifs. Son expertise dans le domaine et sa capacité à rendre les concepts complexes accessibles captivent les élèves, les encourageant à explorer le monde fascinant de la biologie avec intérêt et engagement.
</Text>
       ...
       <View className="flex space-y-2"> 
       <Text className="font-bold">Biologie </Text>
...
export default IsabelleLeblancScreen