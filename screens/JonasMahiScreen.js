...
const JonasMahiScreen = () => {
    
    const navigation = useNavigation();
    const [isFavourite,toggleFavourite] = useState(false);
  return (
    <View className = "bg-white flex-1 ">
    <Image source={require("../assets/Jonas Mahi.jpg")}
    ...
    <FontAwesome name="heart-o" size={24} color={isFavourite? "red" : "black"} />
     ...
     <View  style={{borderTopLeftRadius:40,borderTopRightRadius: 40}} className="px-5  flex flex-1 justify-between bg-white pt-8 -mt-14 ">
      <ScrollView showsVerticalScrollIndicator={false} className="space-y-5">
        ...
        <Text style={{fontSize:15}} className="font-bold flex-1 "> Jonas Mahi </Text>
        <Text style={{fontSize:15}} className="text-[#BA68C8] font-bold"> 2000DA/Mois</Text>
        </View>
        <Text style={{fontSize:15}} className="tracking-wide mb-2">  Jonas Mahi, érudit de la philosophie, délivre des enseignements enchanteurs et captivants. Sa passion pour la pensée profonde et son approche pédagogique élégante invitent les élèves à plonger dans les méandres de la réflexion, éveillant ainsi leur esprit critique et leur sens de la compréhension du monde.
</Text>
       ...
       <View className="flex space-y-2"> 
       <Text className="font-bold">Philosophie </Text>
...
export default JonasMahiScreen