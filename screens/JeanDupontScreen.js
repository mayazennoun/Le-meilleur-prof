...
const JeanDupontScreen = () => {
    
    const navigation = useNavigation();
    const [isFavourite,toggleFavourite] = useState(false);
  return (
    <View className = "bg-white flex-1 ">
    <Image source={require("../assets/profDupontPhoto .jpg")}
    ...
    <FontAwesome name="heart-o" size={24} color={isFavourite? "red" : "black"} />
     ...
     <View  style={{borderTopLeftRadius:40,borderTopRightRadius: 40}} className="px-5  flex flex-1 justify-between bg-white pt-8 -mt-14 ">
      <ScrollView showsVerticalScrollIndicator={false} className="space-y-5">
        ...
        <Text style={{fontSize:15}} className="font-bold flex-1 "> Jean Dupont </Text>
        <Text style={{fontSize:15}} className="text-[#BA68C8] font-bold"> 200DA/H</Text>
        </View>
        <Text style={{fontSize:15}} className="tracking-wide mb-2"> Jean Dupont offre des cours de soutien en mathématiques pour les collégiens et lycéens. Son approche interactive et son dévouement permettent aux étudiants de surmonter les obstacles et d'atteindre leurs objectifs académiques.
</Text>
       <View className="flex-row justify-between mx-1"> 
       <View className="flex-row space-x-2 items-start"> 
       <FontAwesome name="clock-o" size={24} color="#AFA4CE" />
       <View className="flex space-y-2"> 
       <Text className="font-bold">10 Jours</Text>
       ...
       <View className="flex space-y-2"> 
       <Text className="font-bold">Mathématiques </Text>
       ...
       <View className="flex space-y-2"> 
       <Text className="font-bold">200DA/H </Text>
...
export default JeanDupontScreen